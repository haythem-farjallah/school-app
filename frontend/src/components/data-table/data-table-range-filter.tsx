...
export function DataTableRangeFilter<TData>({
  ...
  return (
    <div
      data-slot="range"
      className={cn("flex w-full items-center gap-3 p-3 bg-gradient-to-r from-slate-50/80 to-blue-50/40 rounded-lg border border-slate-200/60", className)}
      {...props}
    >
      <div className="flex-1">
        <label className="text-xs font-medium text-slate-600 mb-1 block">Min</label>
        <Input
          id={`${inputId}-min`}
          type="number"
          ...
          min={min}
          max={max}
          className="h-10 w-full rounded-lg border-slate-300/80 focus:border-blue-400 focus:ring-2 focus:ring-blue-200/60 text-center font-mono"
      ...
      <div className="flex-1">
        <label className="text-xs font-medium text-slate-600 mb-1 block">Max</label>
        <Input
          id={`${inputId}-max`}
          type="number"
