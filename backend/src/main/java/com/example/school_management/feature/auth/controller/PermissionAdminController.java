...
@PreAuthorize("hasRole('ADMIN') and hasAuthority('PERMISSIONS_MANAGE')")
...
public class PermissionAdminController {
    ...
    public ResponseEntity<ApiSuccessResponse<Set<String>>> listAll() {
        var payload = new ApiSuccessResponse<>("success", permissionService.listAllCodes());
        return ResponseEntity.ok(payload);
    }

    // role defaults
    @GetMapping("/roles/{role}")
    public ResponseEntity<ApiSuccessResponse<Set<String>>> showRole(@PathVariable UserRole role) {
        var payload = new ApiSuccessResponse<>("success", permissionService.getRoleDefaults(role));
        return ResponseEntity.ok(payload);
    }
    ...
    // per-user overrides
    @GetMapping("/users/{id}")
    public ResponseEntity<ApiSuccessResponse<Set<String>>> showUser(@PathVariable long id) {
        var payload = new ApiSuccessResponse<>("success", permissionService.getUserPerms(id));
        return ResponseEntity.ok(payload);
    }
